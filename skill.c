...
boolean ValidasiLevelUpBangunan (Bangunan *X){
    return ( JumlahPasukan(Bangunan(Info(P))) >= M(Bangunan(Info(P)))/2);
}
...
//FS: Semua bangunan pemain mengalami level up
    ...
    //Algoritma
    addr = First(Bangunan(*P)); //bangunan ini list
    while (addr != Nil){
        LevelUpBangunan(Bangunan(Info(addr))); // bangunan ini building di adt bangunan
        addr = Next(addr);
    }
}
